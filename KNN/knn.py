...
class KNNApp:
    ...
    def predict(self):
        ...
        if k <= 0:
            messagebox.showerror("Error", "K must be greater than 0.")
            return

        features = []
        labels = []
        for feat_entry, label_entry in zip(self.feature_entries, self.label_entries):
            feat_text = feat_entry.get().strip()
            label_text = label_entry.get().strip()
            if not feat_text or not label_text:
                continue
            try:
                vector = [float(x) for x in feat_text.split(",")]
            except ValueError:
                messagebox.showerror("Error", f"Invalid feature vector: {feat_text}")
                return
            features.append(vector)
            labels.append(label_text)

        if not features:
            messagebox.showerror("Error", "Please enter at least one feature vector and label.")
            return

        try:
            predict_vector = [float(x) for x in self.predict_entry.get().strip().split(",")]
        except ValueError:
            messagebox.showerror("Error", "Invalid prediction vector.")
            return
        ...
        if X.ndim != 2 or X.shape[0] < k:
            messagebox.showerror("Error", "K must be less than or equal to number of samples.")
            return

        if X.shape[1] != len(predict_vector):
            messagebox.showerror("Error", "Prediction vector size does not match feature size.")
            return

        distances = np.linalg.norm(X - np.array(predict_vector), axis=1)
        ...
        neighbors_info = ", ".join([f"{lbl} (d={distances[i]:.3f})" for i, lbl in zip(nearest_idx, nearest_labels)])

        self.result_label.config(text=f"Tahmin Sonucu: {prediction}")
        self.neighbors_label.config(text=f"En Yakin Komsular: {neighbors_info}")
